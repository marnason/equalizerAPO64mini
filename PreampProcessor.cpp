...
PreampProcessor::PreampProcessor()
	: inputChannelCount(0), outputChannelCount(0), gain(1.0)
{
}

void PreampProcessor::initialize(unsigned inputChannels, unsigned outputChannels)
{
...
{
	gain.store(std::pow(10.0, dbGain / 20.0), std::memory_order_release);
}

#pragma AVRT_CODE_BEGIN
void PreampProcessor::process(float* output, const float* input, unsigned frameCount) const
{
	const double gainFactor = gain.load(std::memory_order_acquire);

	if (gainFactor == 1.0 && inputChannelCount == outputChannelCount)
	{
		if (output != input)
